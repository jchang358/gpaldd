function vVar = td3(trialnum, choice, ID, subdir)
    % ADO module for dedlay discounting using a scaled hyperbolic model which scales k by the ratio (Ass/All)^a
    trialnum = floor(trialnum)-1;
    %1 or 0, o is left
    choice = floor(choice);

    fVar = struct('numpractice',4,'numparmest',30,'aHBmin',0,'aHBmax',100,...
    ...
    if trialnum==-1 || trialnum==-2
        ...
        if trialnum+1 > fVar.numpractice
            ...
            scaling = exp(fVar.kHBparms).*(vVar.design(1)./vVar.design(3)).^fVar.aHBparms;
            uSSmat = vVar.design(1).*(( exp(fVar.kHBparms).*scaling.*vVar.design(2)+1).^-1);
            uLLmat = vVar.design(3).*(( exp(fVar.kHBparms).*scaling.*vVar.design(4)+1).^-1);
    ...
    if trialnum+1 <= fVar.numpractice
        ...
        scaling_u = repmat(exp(tmp_kHBp),fVar.nnn,1).*(repmat(vSS./vLL,1,numidx)).^repmat(tmp_aHBp,fVar.nnn,1);
        
        uSS = repmat(vSS,1,numidx).*scaling_u.*(tSS*exp(tmp_kHBp)+1).^-1;
        uLL = repmat(vLL,1,numidx).*scaling_u.* (tLL*exp(tmp_kHBp)+1).^-1;
