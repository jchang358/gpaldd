function vVar = td(trialnum, choice, ID, subdir)
    % ADO module for dedlay discounting using the standard hyperbolic model
    
    trialnum = floor(trialnum)-1;
    %1 or 0, o is left
    choice = floor(choice);
    %rng(88);
    fVar = struct('numpractice',0,'numparmest',25,'kHBmin',log(0.0001),...
                    'kHBmax',log(0.99),'eHBmin',0.0001,'eHBmax',0.5,'kHBgran',80,...%45
    ...
    if trialnum==-1 || trialnum==-2
        ...
        [fVar.kHBparms, fVar.eHBparms] = meshgrid(fVar.kHB(2:end), fVar.eHB(2:end));
        fVar.kHBparms = transpose(fVar.kHBparms);
        fVar.eHBparms = transpose(fVar.eHBparms);
        fVar.nSamp = 1000;
        
        c = (fVar.eHBgran+1)*(fVar.kHBgran+1);
        vVar.HBprior=ones(fVar.kHBgran+1,fVar.eHBgran+1)/c;
        ...
        fVar.ddisp = {[  0,    0.143,  0.286,   0.429,  0.571,  0.714,   0.857,...
               1,      1.143,  1.286,   1.428,  1.571,  1.714,   1.857,    2,...
               ...
               'in 11 days', 'in 12 days','in 13 days', 'in 14 days', 'in 16 days',...
        ...
        if trialnum+1 > fVar.numpractice
            ...
            py1theta = (1+exp(fVar.eHBparms.*(uSSmat - uLLmat))).^-1;
        ...
        if trialnum+1 > fVar.numpractice
            sumHBpost2 = sum(vVar.HBpost,2);
            sumHBpost1 = sum(vVar.HBpost,1);
    ...
    if trialnum+1 <= fVar.numpractice
        ...
        [paramidx, cnt] = categrnd(vVar.HBpost, fVar.nSamp);
        tmp_kHBp = reshape(fVar.kHBparms', 1, numel(fVar.kHBparms));
        tmp_eHBp = reshape(fVar.eHBparms', 1, numel(fVar.eHBparms));
        ...
        py1theta = (1+exp((uSS-uLL).*repmat(tmp_eHBp,fVar.nnn,1))).^-1;
